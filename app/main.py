...
async def get_contacts(
    ...
    """
    contacts = db.query(Contact).filter(Contact.deletedAt.is_(None)).offset(skip).limit(limit).all()
    return contacts
...
def _find_matching_contacts(email: str = None, phoneNumber: str = None, db: Session = None) -> list[Contact]:
    """
...
def _get_all_related_contacts(matching_contacts: list[Contact], db: Session) -> set[Contact]:
    """
    ...
    for contact in matching_contacts:
        if contact.linkPrecedence == LinkPrecedence.SECONDARY and contact.linkedId:
            ...
            if primary:
                ...
                other_secondaries = db.query(Contact).filter(
...
def _merge_contact_groups(primary_contacts: list[Contact], db: Session) -> Contact:
    ...
    for losing_primary in losing_primaries:
        ...
        losing_secondaries = db.query(Contact).filter(
...
def _create_secondary_contact(email: str, phoneNumber: str, primary_contact: Contact, db: Session) -> Contact:
    """
...
def _collect_consolidated_info(all_contacts: set[Contact], primary_contact: Contact) -> tuple[list[str], list[str], list[int]]:
    """
...
def identify_contact(email: str = None, phoneNumber: str = None, db: Session = None) -> IdentifyResponse:
    """
    ...
    try:
        ...
        if not exact_match_exists:
            new_secondary = _create_secondary_contact(email, phoneNumber, winning_primary, db)
        ...
        for contact in all_related_contacts:
            if contact.id != winning_primary.id:
                if contact.linkPrecedence != LinkPrecedence.SECONDARY or contact.linkedId != winning_primary.id:
